...
class MaskFormer(nn.Module):
    ...
    def __init__(
        ...
        # contrastive
        self.contrastive_on = contrastive_on
        ...
        if self.contrastive_on:
            ...
            self.con_loss_calculator = ContrastiveLoss(self.con_embedding_layer, self.ignore_occlusion_label,
    ...
    def from_config(cls, cfg):
        ...
        if cfg.CONTRASTIVE.ON:
            con_weight = cfg.CONTRASTIVE.CONTRASTIVE_WEIGHT
        ...
        if deep_supervision:
            ...
            if cfg.CONTRASTIVE.ON:
                weight_dict["con_loss"] = con_weight
        ...
        return {
            ...
            "inner_threshold_on": cfg.CONTRASTIVE.INNER_THRESHOLD_ON,
            "inner_threshold": cfg.CONTRASTIVE.INNER_THRESHOLD,
            "inner_occlusion_label": cfg.CONTRASTIVE.INNER_OCCLUSION_LABEL,
        }
    ...
    def forward(self, batched_inputs):
        ...
        # name = batched_inputs[0]["file_name"].split('/')[-1].split('.')[0]
        # feature = F.adaptive_avg_pool2d(features['res5'], (1, 1))
        # feature = feature.view(len(batched_inputs), -1)
        # self.feature_pth['features'].append(feature)
        # occlusion_label = self.occlusion_label_val_dict[name]
        ...
        if self.training:
            ...
            if self.contrastive_on:
                labels = torch.tensor(
                    [self.label_mapping[self.occlusion_labels[_input['file_name'][-16:-4]]] for _input in
                     batched_inputs],
                    device=self.device
                )
                losses['con_loss'] = self.con_loss_calculator.compute_loss(features, labels)
            ...
            # Ensure no intermediate tensors are kept
            return losses
        else:
